...
north={"Move forward":(0,1), "Move backward":(0,-1), "Turn left":(-1,0), "Turn right":(1,0)}
east={"Move forward":(1,0), "Move backward":(-1,0), "Turn left":(0,1), "Turn right":(0,-1)}
south={"Move forward":(0,-1), "Move backward":(0,1), "Turn left":(1,0), "Turn right":(-1,0)}
west={"Move forward":(-1,0), "Move backward":(1,0), "Turn left":(0,-1), "Turn right":(0,1)}
def nov_facing(facing,action):
    if facing == "north":
...
class Pacman(Problem):
    ...
    def check_valid(self,state):
        (x,y),facing,dots=state
        walls=self.walls
        if 0<=x<10 and 0<=y<10 and not (x,y) in walls:
            return True
        return False
    def successor(self, state):
        rez={}
        ...
        if facing=="north":
            for action,(dx,dy) in north.items():
                nx,ny=x+dx,y+dy
                nov_pos=(nx,ny)
                n_dots=dots
                if nov_pos in dots:
                    n_dots=list(dots)
                    n_dots.remove(nov_pos)
                    n_dots=tuple(n_dots)
                n_facing=nov_facing(facing,action)
                if self.check_valid((nov_pos,n_facing,n_dots)):
                    rez[action]=(nov_pos,n_facing,n_dots)
        elif facing=="east":
            for action, (dx, dy) in east.items():
                n_dots=dots
                nx, ny = x + dx, y + dy
                nov_pos = (nx, ny)
                if nov_pos in dots:
            ...
            for action, (dx, dy) in south.items():
                n_dots=dots
                nx, ny = x + dx, y + dy
                nov_pos = (nx, ny)
                if nov_pos in dots:
            ...
            for action, (dx, dy) in west.items():
                n_dots=dots
                nx, ny = x + dx, y + dy
                nov_pos = (nx, ny)
                if nov_pos in dots:
...
if __name__ == "__main__":
    walls=((0,9),(1,9),(2,9),(3,9),(6,9),(0,8),(8,8),(9,8),(4,7),(9,7),(8,7),(0,6),(3,6),(4,6),(5,6),(4,5),(1,4),(8,4),(9,4),(1,3),(1,2),(6,2),(4,1),(5,1),(6,1),(8,1),(6,0))
    x=int(input())
    y=int(input())
    pos=(x,y)
    direction=input()
    n=int(input())
    blobs_pos=tuple([tuple(map(int,input().split(','))) for _ in range(n)])
    problem=Pacman(walls,(pos,direction,blobs_pos))
