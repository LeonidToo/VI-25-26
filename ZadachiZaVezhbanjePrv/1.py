...
class Box(Problem):
    ...
    def goal_test(self, state):
        covek,k1,k2=state
        return self.end_point==k1==k2

    @staticmethod
    def check_valid(state):
        covek,k1,k2=state
        x, y, x1, y1, x2, y2 = covek[0],covek[1], k1[0],k1[1], k2[0],k2[1]
        if 0<=x<5 and 0<=y<5 and 0<=x1<5 and 0<=y1<5 and 0<=x2<5 and 0<=y2<5 and k1!=k2:
            return True
        if 0 <= x < 5 and 0 <= y < 5 and 0 <= x1 < 5 and 0 <= y1 < 5 and 0 <= x2 < 5 and 0 <= y2 < 5 and k1 == k2 == (4,4):
            return True
        return False
    ...
    def successor(self, state):
        ...
        for action,(dx,dy) in dir.items():
            nx,ny=x+dx,y+dy
            n_pos=(nx,ny)
            if n_pos==k1 and k1!=self.end_point:
                nx1, ny1 = x1 + dx, y1 + dy
                n_k1=(nx1,ny1)
                if self.check_valid((n_pos,n_k1,k2)):
                    succ["Push box 1 "+action]=(n_pos,n_k1,k2)
            elif n_pos==k2 and k2!=self.end_point:
                nx2, ny2 = x2 + dx, y2 + dy
                n_k2=(nx2,ny2)
                if self.check_valid((n_pos,k1,n_k2)):
                    succ["Push box 2 "+action]=(n_pos,k1,n_k2)
            else:
                if self.check_valid((n_pos,k1,k2)):
                    succ["Move man "+action]=(n_pos,k1,k2)
        return succ
...
if __name__ == '__main__':
    ...
    rez=breadth_first_graph_search(box)
    if rez:
        print(rez.solution())
    else:
        print("No Solution!")
