...
Movement={"R1: Disk ":1,"R2: Disk ":2,"L1: Disk ":-1,"L2: Disk ":-2}

class Lenti(Problem):
    def __init__(self,n,l,initial,goal=None):
    ...
    def successor(self, state):
        ...
        n=self.n
        l=self.l
        # I ovoa e tochno samo naogja drugo reshenie od toa na courses, oti direktno iterira so 1,2,3...n mesto da ide niz
        # nizata i da najde prv broj koj mozhe da go premestuva, vaka sekogash kje se dodat prvo potezite na 1 pa 2 itn.
        # mesto koj prv e vo nizata
        # for i in range(1,n+1):
        #     for action,di in Movement.items():
        #         nlenta=list(state)
        #         index=lenta.index(i)
        #         ni=index+di
        #         if 0<=ni<l:
        #             if (di==2 and (nlenta[ni-1]<1 or nlenta[ni]>0)) or (di==-2 and (nlenta[ni+1]<1 or nlenta[ni]>0)) or (not 0<=ni<l) or (di==1 and nlenta[ni]>0) or (di==-1 and nlenta[ni]>0):
        #                 continue
        #         else:
        #             continue
        #         nlenta[index]=-1
        ...
        for i in range(l):
            if lenta[i]!=-1:
                for action, di in Movement.items():
                    ...
                    ni = index + di
                    if 0 <= ni < l:
                        if (di == 2 and (nlenta[ni - 1] < 1 or nlenta[ni] > 0)) or (
                                di == -2 and (nlenta[ni + 1] < 1 or nlenta[ni] > 0)) or (not 0 <= ni < l) or (
                                di == 1 and nlenta[ni] > 0) or (di == -1 and nlenta[ni] > 0):
                            continue
                    else:
                        continue
                    nlenta[index] = -1
    ...
    def goal_test(self, state):
        lenta=list(state)
        for i in range(1,n+1):
            if not lenta.index(i)==self.l-i:
                return False
        return True
if __name__ == "__main__":
    n=int(input())
    l=int(input())
    diskovi=[i for i in range(1,n+1)]
    lenta=tuple([i if i<=n else -1 for i in range(1,l+1)])
    problem=Lenti(n,l,lenta)
