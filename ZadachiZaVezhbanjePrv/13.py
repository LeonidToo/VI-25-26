...
Movement={"Up 1":(0,1),"Up 2":(0,2),"Up 3":(0,3),"Right 1":(1,0),"Right 2":(2,0),"Right 3":(3,0)}
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        (x,y)=state
        pos=(x,y)
        if pos in walls or not 0<=x<n or not 0<=y<n:
            return False
        return True
    ...
    def successor(self, state):
        ...
        for action,(dx,dy) in Movement.items():
            nx,ny=x+dx,y+dy
            if self.check_valid((nx,ny),self.walls,self.n):
