...
class Problem:
    ...
    def successor(self, state):
        ...
        walls=self.walls
        n=self.n
        for (x,y) in balls_pos:
            for action, (dx, dy) in direction.items():
                nx,ny=x+dx,y+dy
                if (nx,ny) in balls_pos and not ((nx+dx,ny+dy) in walls) and 0<=nx+dx<n and 0<=ny+dy<n:
                    n_balls_pos_list=list(balls_pos)
                    n_balls_pos_list.remove((x,y))
                    n_balls_pos_list.remove((nx,ny))
                    nx, ny = nx + dx, ny + dy
                    n_balls_pos_list.insert(0,(nx,ny))
                    n_balls_pos=tuple(n_balls_pos_list)
                    rez[action+f'(x={x},y={y})']=n_balls_pos
        return rez
...
if __name__ == "__main__":
    n=int(input())
    k=int(input())
    balls_pos=tuple([tuple(map(int,input().split(','))) for _ in range(k)])
    k=int(input())
    walls_pos=tuple([tuple(map(int,input().split(','))) for _ in range(k)])

    problem=Problem(walls_pos, n, balls_pos)
