...
class Zmijche(Problem):
    def __init__(self,crveni_jabuki,initial,goal=None):
        super().__init__(initial,goal)
        self.crveni_jabuki=crveni_jabuki

    def check_valid(self,state):
        zmija,(dx,dy),zeleni_jabuki=state
        (zx,zy)=zmija[-1]
        zmija=list(zmija)
        zmija.pop(-1)
        zmija=tuple(zmija)
        if 0<=zx<10 and 0<=zy<10 and not (zx,zy) in self.crveni_jabuki and not (zx,zy) in zmija:
            return True
        return False
    def successor(self, state):
        rez={}
        zmija,(dx,dy),zeleni_jabuki=state
        (zx,zy)=zmija[-1]
        Movement={"Move forward":(dx,dy),"Turn right":(dy,-dx),"Turn left":(-dy,dx)}
        for action,(dx,dy) in Movement.items():
            nx,ny=zx+dx,zy+dy
            nov_pos=(nx,ny)
            n_zmija=list(zmija)
            n_zeleni_jabuki=list(zeleni_jabuki)
            if nov_pos in zeleni_jabuki:
                n_zeleni_jabuki.remove(nov_pos)
                n_zmija.append(nov_pos)
            else:
                n_zmija.pop(0)
                n_zmija.append(nov_pos)
            n_zmija=tuple(n_zmija)
            n_zeleni_jabuki=tuple(n_zeleni_jabuki)
            if self.check_valid((n_zmija,(dx,dy),n_zeleni_jabuki)):
                rez[action]=(n_zmija,(dx,dy),n_zeleni_jabuki)
        return rez
    def actions(self, state):
...
if __name__ == '__main__':
    ...
    nasoka=(0,-1)
    n=int(input())
    zeleni_jabuki=tuple([tuple(map(int,input().split(','))) for _ in range(n)])
    n=int(input())
    crveni_jabuki=tuple([tuple(map(int,input().split(','))) for _ in range(n)])
    problem=Zmijche(crveni_jabuki,(zmija,nasoka,zeleni_jabuki))
