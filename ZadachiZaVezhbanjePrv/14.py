...
class Squares(Problem):
    ...
    def h(self, node):
        (x1,y1),(x2,y2),(x3,y3),(x4,y4),(x5,y5)=node.state
        evristika=abs(0-x1)+abs(4-y1)
        evristika+=abs(1-x2)+abs(3-y2)
        evristika+=abs(2-x3)+abs(2-y3)
        evristika+=abs(3-x4)+abs(1-y4)
        evristika+=abs(4-x5)+abs(0-y5)
        return evristika
